...
func (ch *Checker) checkSite(site models.Site) {
	...
	// httpSkipped is set to true when we intentionally omit the HTTP probe so
	// that a zero HTTPStatus is not misread as a network failure below.
	var httpSkipped bool

	// HTTP check
	if site.IsLocal {
		// For local sites we probe the upstream directly instead of hitting the
		// public domain, which may be behind Cloudflare or only resolvable on
		// the local network (e.g. *.local domains).
		probeURL := localProbeURL(site)
		if probeURL == "" {
			// No usable local endpoint — skip the HTTP probe and rely solely
			// on container status (e.g. shared-infra.local).
			httpSkipped = true
		} else {
			start := time.Now()
			resp, err := ch.Client.Get(probeURL)
	...
	// (status 0), is treated as the site being down. When the HTTP probe was
	// intentionally skipped (local site with no usable endpoint) a zero status
	// is not a failure — health is determined by container status alone.
	httpDown := !httpSkipped && (hc.HTTPStatus == 0 || hc.HTTPStatus >= 400)
	isDown := httpDown || hc.ContainerStatus == "not_found" || hc.ContainerStatus == "exited"
...
// localProbeURL returns the URL to probe for a local site, bypassing the
// public domain (which may be behind Cloudflare or unresolvable externally).
// Returns an empty string when no usable local endpoint can be determined,
// which signals that the HTTP probe should be skipped entirely.
func localProbeURL(site models.Site) string {
	// Explicit port wins — fast path used by sites like gitlab.weeniesmp.net.
	if site.Port > 0 {
	...
	// upstreams that are genuinely local — remote IPs (e.g. 45.126.38.75) are
	...
	if site.RoutingConfig != nil && len(site.RoutingConfig.Rules) > 0 {
		...
		chosen := rules[len(rules)-1] // default: last rule
		for _, r := range rules {
			if r.PathPrefix == "" {
				chosen = r
				break
		...
		// Upstream is a remote host — skip the HTTP probe.
		return ""
	}

	// No port and no routing config — skip the HTTP probe.
	return ""
}
